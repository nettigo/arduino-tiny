...
private:
  ...
  void _reset()
  {
    ...
    _protocol.registerWrite(0x03, 0b00000011);
    ...
    byte address1[5] = {0xe7, 0xe7, 0xe7, 0xe7, 0xe7};
    byte address2[5] = {0xc2, 0xc2, 0xc2, 0xc2, 0xc2};
    _protocol.addressWrite(0x0a, address1, 5);
    _protocol.addressWrite(0x0b, address2, 5);
    ...
    // tx address
    _protocol.addressWrite(0x10, address1, 5);
    // paylo9ad size 0..5
...
public:
  ...
  void txAddress(uint8_t address[5])
  {
    _protocol.addressWrite(0x0a, address, 5);
    _protocol.addressWrite(0x10, address, 5);
  }

  void rxAddress(uint8_t address[5])
  {
    _protocol.addressWrite(0x0b, address, 5);
  }
...
public:
  ...
  void begin(uint8_t address[5], uint8_t channel, uint8_t csnPin=3, uint8_t cePin=7)
  {
    device.begin(csnPin);
    _ce.begin(cePin);
  ...
  void write(uint8_t address[5], uint8_t *data, uint8_t size)
  {
    _ce.deactivate();
    device.txMode();
    _powerOnDelay();
  ...
  template <typename T>
  void write(uint8_t address[5], T &t)
  {
  ...
  uint8_t flush()
  {
    uint8_t status = device.txStatus();
    return status;
  }
